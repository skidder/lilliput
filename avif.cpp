...
avif_decoder avif_decoder_create(const opencv_mat buf) {
    ...
    d->decoder->image->colorPrimaries = AVIF_COLOR_PRIMARIES_BT709;
    d->decoder->image->transferCharacteristics = AVIF_TRANSFER_CHARACTERISTICS_SRGB;
    d->decoder->image->matrixCoefficients = AVIF_MATRIX_COEFFICIENTS_BT709;
    d->decoder->image->yuvRange = AVIF_RANGE_FULL;
...
size_t avif_encoder_write(avif_encoder e, const opencv_mat src, const int* opt, size_t opt_len, int delay_ms, int blend, int dispose) {
    ...
    if (e->icc && e->icc_len > 0 && e->frame_count == 0) {
        avifImageSetProfileICC(avifImage, e->icc, e->icc_len);
    }
...
uint32_t avif_decoder_get_bgcolor(const avif_decoder d) {
    if (!d || !d->decoder) {
        return 0xFFFFFFFF;  // Default to white background
    }
    return d->bgcolor;
}
